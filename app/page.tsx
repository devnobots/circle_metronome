...
export default function Metronome() {
  ...
  useEffect(() => {
    ...
    const currentSessionCount = storedSessionCount
      ? Number.parseInt(storedSessionCount)
      : 0;
  ...
  const animateMetronome = () => {
    const animate = (timestamp: number) => {
      ...
      if (elapsedSinceLastBeat >= beatDuration) {
        playClickTone();
        const beatsSinceLastBeat = Math.floor(
          elapsedSinceLastBeat / beatDuration
        );
        lastBeatTimeRef.current =
          lastBeatTimeRef.current + beatsSinceLastBeat * beatDuration;
      }
      ...
      {!showUpgradeMessage ? (
        <div className={styles.controlsContainer}>
          {showTapButton ? (
            ...
            <button
              className={styles.tapButtonAbove}
              onClick={startTapMode}
              aria-label="Start tap tempo"
            >
              TAP
            </button>
          )}
        ...
        <div className={styles.upgradeMessage}>
          ...
          <p className={styles.upgradeBody}>
            You've experienced The Circular Metronome's fluid, uninterrupted
            motion. Upgrade to enjoy unlimited sessions without interruption.
            You can also continue using the metronome for free, though sessions
          ...
          <div className={styles.upgradeButtons}>
            ...
            <button
              id="five-minute-sessions-button"
              className={styles.standardButton}
              onClick={handleFiveMinuteSessions}
            >
              5 Minute Sessions
            </button>
          </div>
